"
This exercise features an app where the user can select a random sample 
of desired title types. The app then reports the frequencies of various 
title types in the sample and plots user selected variables for these data. 
There are two new functions of note in the server: observeEvent() and updateNumericInput().
We use them to update the numeric input widget to display a maximum allowed sample size
based on the selected title types. For example, if you only select "TV Movie", the maximum 
allowed sample size is 5, because there are only 5 TV movies in the sample. We'll learn more 
about observeEvent() later in the course. For now, your task is simple: find the mismatched 
use of reactives.
"
